...
const InvitationManager: React.FC<InvitationManagerProps> = ({ siteUrl }) => {
  ...
  const [statusMsg, setСтатусMsg] = useState("");
  ...
  const generatePDFDoc = async (guest: GuestData): Promise<jsPDF> => {
    ...
    const salamOpening =
      cfg.TEXT_SALAM_OPENING || "Ассалому алайкум ва рахматуллохи ва баракотух";
    ...
    const signature = cfg.TEXT_SIGNATURE || "С уважением и радостью,";
    ...
    doc.text("СВАДЬБА", cx, 50, { align: "center" });
    ...
    doc.setFont("times", "normal");
    doc.setFontSize(9);
    doc.text("Уважаемый(ая) гость:", cx, boxY + 8, {
      align: "center",
    });
    ...
    doc.setFontSize(10);
    doc.text(
      "Мы рады сообщить о свадьбе наших детей:",
      cx,
      95,
      { align: "center" }
    );
    ...
    doc.text("Иншааллах, торжество состоится:", cx, 35, {
      align: "center",
    });
    doc.setFont("times", "bold");
    doc.setFontSize(14);
    ...
    doc.text("МЕСТО ПРОВЕДЕНИЯ:", cx, 115, { align: "center" });
    doc.setFont("times", "normal");
    doc.setFontSize(11);
    ...
    doc.text("Сканируйте, чтобы открыть карту", cx, qrY + qrSize + 5, { align: "center" });
    ...
    doc.text("ЭЛЕКТРОННОЕ ПРИГЛАШЕНИЕ", cx, 50, { align: "center" });
    const qrDigiSize = 40;
    const digiY = 60;
    doc.setFillColor(
      currentTheme.bg[0],
    ...
    doc.text(
      "Отсканируйте, чтобы открыть приглашение и подтвердить участие",
      cx,
      digiY + qrDigiSize + 8,
      { align: "center" }
    );
  ...
  const downloadTemplate = () => {
    const csvContent =
      "Имя гостя,Адрес (необязательно)\nBapak Jokowi & Ibu Iriana,Jakarta\nTeman-teman Alumni SMA 1,На месте\nKeluarga Besar H. Syarif,Bandung";
      ...
      Papa.parse(file, {
        complete: (results) => {
          ...
          results.data.forEach((row: any) => {
            const name = row[0];
            const address = row[1] || "На месте";
            if (name && typeof name === "string" && name.trim() !== "") {
              if (!name.toLowerCase().includes("имя гостя")) {
  ...
  const processBulk = async () => {
    ...
    try {
      ...
      for (let i = 0; i < bulkData.length; i += CHUNK_SIZE) {
        ...
        setСтатусMsg(
          `Готовим PDF ${i + 1}–${Math.min(i + chunk.length, bulkData.length)} из ${bulkData.length}…`
        );
        await Promise.all(
      ...
      setСтатусMsg("Упаковываем ZIP-архив…");
      const content = await zip.generateAsync({ type: "blob" });
      handleSaveAs(
        content,
        `Priglasheniya-${currentTheme.id}-${new Date().toISOString().slice(0, 10)}.zip`
      );
      setСтатусMsg("Готово!");
    } catch (error) {
    ...
    <div className="animate-reveal space-y-8">
      <div className="space-y-3 text-center">
        ...
        <p className="mx-auto max-w-2xl text-sm leading-relaxed text-slate-600 dark:text-slate-400">
          Элегантный флористический шаблон (4 страницы A5): векторный орнамент, QR-код и
          Multi-Theme Support
        </p>
      </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="Например: семья Юсуповых"
            ...
            <div className="space-y-6 rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-800/50">
              ...
              <div className="space-y-2">
                ...
                <p className="mx-auto max-w-md text-sm text-slate-500">
                  Подготовьте CSV-файл такого формата:
                  <br />
                  <strong>Колонка 1: имя гостя</strong> |{" "}
                  <strong>Колонка 2: адрес / город (необязательно)</strong>
                </p>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    <div className="relative z-10 flex items-center gap-2">
                      ...
                      <span>
                        {isProcessing ? "Обрабатываем…" : "Download ZIP"}
                      </span>
                    </div>
                  </button>
                </div>
