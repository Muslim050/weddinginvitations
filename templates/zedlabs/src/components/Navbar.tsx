...
const Navbar: React.FC<NavbarProps> = ({ theme, toggleTheme }) => {
  ...
  const navItems = [
    ...
    { icon: User, label: t.nav.couple, href: "#couple" },
    { icon: Calendar, label: t.nav.events, href: "#event" },
    { icon: ImageIcon, label: t.nav.gallery, href: "#gallery" },
    { icon: Send, label: t.nav.rsvp, href: "#rsvp" },
    { icon: MessageCircle, label: t.nav.wishes, href: "#wishes" },
  ];
  const itemBaseClass =
  ...
  return (
    ...
    >
      <div className="dark:bg-darkSurface/60 pointer-events-auto flex items-center justify-between gap-1 rounded-full border border-white/40 bg-white/60 p-1.5 shadow-[0_20px_50px_rgba(0,0,0,0.2)] backdrop-blur-2xl transition-colors duration-1000 md:justify-center md:p-2 dark:border-white/10">
        ...
        >
          <span className="text-[11px] font-black tracking-widest md:text-[13px]">
            {lang.toUpperCase()}
          </span>
