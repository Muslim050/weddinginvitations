...
const DataTable = <T extends { id: number }>({
  ...
  const executeBulkDelete = () => {
    if (onBulkDelete && selected.length > 0) {
      if (confirm(`Удалить выбранные записи (${selected.length})?`)) {
    ...
    <div className="space-y-4">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div className="flex items-center gap-2">
          ...
          {onBulkDelete && selected.length > 0 && (
            ...
            >
              <Trash2 className="h-3.5 w-3.5" /> Удалить ({selected.length})
            </button>
          )}
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="text-xs text-slate-500">
          Страница {filteredData.length === 0 ? 0 : page} из {totalPages}{" "}
...
const CONFIG_SECTIONS: { title: string; fields: ConfigField[] }[] = [
  {
    title: "Невеста",
    fields: [
      { key: "BRIDE_NICKNAME", label: "Короткое имя", type: "text" },
      { key: "BRIDE_FULLNAME", label: "Полное имя", type: "text" },
      { key: "BRIDE_PARENTS", label: "Родители", type: "text" },
      { key: "BRIDE_INSTAGRAM", label: "Instagram (без @)", type: "text" },
      { key: "BRIDE_IMAGE", label: "URL фото", type: "url" },
    ],
  },
  {
    title: "Жених",
    fields: [
      { key: "GROOM_NICKNAME", label: "Короткое имя", type: "text" },
      { key: "GROOM_FULLNAME", label: "Полное имя", type: "text" },
      { key: "GROOM_PARENTS", label: "Родители", type: "text" },
      { key: "GROOM_INSTAGRAM", label: "Instagram (без @)", type: "text" },
      { key: "GROOM_IMAGE", label: "URL фото", type: "url" },
    ],
  },
  {
    title: "Venue",
    fields: [
      { key: "VENUE_NAME", label: "Название площадки", type: "text" },
      { key: "VENUE_ADDRESS", label: "Полный адрес", type: "textarea" },
      {
        key: "VENUE_LAT",
        label: "Latitude",
        type: "text",
  ...
  },
  {
    title: "Никох",
    fields: [
      { key: "AKAD_TITLE", label: "Название", type: "text" },
      { key: "AKAD_DAY", label: "День недели", type: "text", placeholder: "Воскресенье" },
      {
        key: "AKAD_DATE",
        label: "Дата",
        type: "text",
  ...
  },
  {
    title: "Той",
    fields: [
      { key: "RESEPSI_TITLE", label: "Название", type: "text" },
      {
        key: "RESEPSI_DAY",
        label: "День недели",
        type: "text",
  ...
  },
  {
    ...
    fields: [
      { key: "HERO_IMAGE", label: "URL обложки", type: "url" },
      {
        key: "HERO_CITY",
        label: "Город",
        type: "text",
      ...
      { key: "MUSIC_URL", label: "URL музыки (MP3)", type: "url" },
      {
        key: "RSVP_MAX_GUESTS",
        label: "Макс. гостей в анкете",
        type: "number",
        placeholder: "20",
      },
    ],
  },
  {
    title: "Тексты",
    fields: [
      { key: "TEXT_SALAM_OPENING", label: "Приветствие", type: "text" },
      { key: "TEXT_QUOTE_AR_RUM", label: "Цитата (Ар-Рум)", type: "textarea" },
      {
        key: "TEXT_QUOTE_SOURCE",
        label: "Источник цитаты",
        type: "text",
        placeholder: "Коран, сура Ар-Рум, аят 21",
      },
      { key: "TEXT_INVITATION", label: "Текст приглашения", type: "textarea" },
      { key: "TEXT_CLOSING", label: "Завершающий текст", type: "textarea" },
      { key: "TEXT_SALAM_CLOSING", label: "Прощание", type: "text" },
      {
        key: "TEXT_SIGNATURE",
        label: "Подпись",
        type: "text",
        placeholder: "С уважением и радостью,",
      },
      {
        key: "TEXT_FAMILY",
        label: "Семья",
        type: "text",
        placeholder: "Семья ... и семья ...",
      },
      {
        key: "TEXT_GIFT_TITLE",
        label: "Заголовок блока подарков",
        type: "text",
        placeholder: "Подарок молодожёнам",
      },
      { key: "TEXT_GIFT_DESC", label: "Описание блока подарков", type: "textarea" },
    ],
  },
  {
...
const SettingsTab: React.FC = () => {
  ...
  const validateJson = (key: string, value: string): boolean => {
    try {
      ...
      setJsonErrors((prev) => ({ ...prev, [key]: "Некорректный JSON" }));
      return false;
    }
  };
...
const AdminDashboard = ({
  ...
  const tabs = [
    { id: "rsvp", label: "Ответы гостей", icon: Users },
    { id: "wishes", label: "Пожелания", icon: MessageCircle },
        ...
        <div className="animate-reveal space-y-6">
          ...
          <DataTable
            ...
            columns={[
              ...
              { header: "Гостей", accessor: "guest_count" },
              {
                header: "Сообщение",
                accessor: (item) => (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <select
                        ...
                        className="w-full rounded-lg border p-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                      >
                        <option value="hadir">Придут</option>
                        <option value="ragu">Не уверены</option>
                        <option value="tidak_hadir">Не придут</option>
                      </select>
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase">
