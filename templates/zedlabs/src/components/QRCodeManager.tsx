        ...
        complete: (results) => {
          ...
          results.data.forEach((row: any) => {
            if (row[0] && typeof row[0] === "string" && row[0].trim() !== "") {
              if (!row[0].toLowerCase().includes("имя гостя")) {
  ...
  const downloadAllZip = async () => {
    ...
    try {
      ...
      for (let i = 0; i < bulkNames.length; i += CHUNK_SIZE) {
        ...
        setСтатусMsg(
          `Обрабатываем ${i + 1}–${Math.min(i + chunk.length, bulkNames.length)} из ${bulkNames.length}…`
        );
        await Promise.all(
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">
                      Премиальный дизайн с логотипом
                    </span>
                  </div>
                </div>
                <div className="flex gap-3">
              ...
              {bulkNames.length > 50 && (
                <p className="text-center text-xs text-slate-400 italic">
                  ...
                  чтобы не нагружать браузер, но файл всё равно будет скачан).
                </p>
              )}
            </div>
          )}
